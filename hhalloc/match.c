...
LEFT JOIN match ON hh.hhid=match.hhid WHERE choice=1 AND race =%d",
	...
	for (int i = 0; i < totalRaces; i++)
	{
		...
		printf("%3.5f,", (matchedTotal[i] / numInGroup[i]) * 100.0);
		// Metric 2: Percentage in each group that got first choice
		printf("%3.5f,", (topChoiceNum[i] / matchedTotal[i]) * 100.0);
		// Metric 3 : Average wait time for each racial group
		printf("%3.5f,", (totalWaitTime[i] / matchedTotal[i]) *1.0);
	}
...
char query[MAX_QUERY_LEN + 1];
strlcat(query, "SELECT * from hh LEFT JOIN match ON hh.hhid = match.hhid ORDER BY RANDOM()", sizeof(query));
